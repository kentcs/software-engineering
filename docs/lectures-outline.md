# 8-Lecture Outline (Post-Break / Advanced Topics)

## Lecture 13: Configuration and Version Management (Post-Deployment)
- SWEBOK: Software Configuration Management
- Topics:
  - Versioning strategies
  - Build/release concepts
  - Traceability from commit to deployment
- Head First Software Development:
  - Chapter 6: Version Control: Defensive development
  - Chapter 6.5: Building your Code: Insert tab a into slot b...
  - Chapter 7: Testing and Continuous Integration: Things fall apart
- Wikipedia:
  - https://en.wikipedia.org/wiki/Software_configuration_management
  - https://en.wikipedia.org/wiki/Version_control
  - https://en.wikipedia.org/wiki/Continuous_delivery

## Lecture 14: Software Maintenance and Evolution
- SWEBOK: Software Maintenance
- Topics:
  - Corrective, adaptive, perfective maintenance
  - Refactoring in practice
  - Technical debt
- Head First Software Development:
  - Chapter 5: Good-Enough Design: Getting it done with great design
  - Chapter 7: Testing and Continuous Integration: Things fall apart
  - Chapter 8: Test-Driven Development: Holding your code accountable
  - Chapter 9: Ending an Iteration: It’s all coming together...
  - Appendix A: Leftovers: The top 5 topics (we didn’t cover)
    - Refactoring
- Wikipedia:
  - https://en.wikipedia.org/wiki/Software_maintenance
  - https://en.wikipedia.org/wiki/Technical_debt
  - https://en.wikipedia.org/wiki/Code_refactoring

## Lecture 15: Software Architecture (Beyond MVC)
- SWEBOK: Software Architecture
- Topics:
  - Architectural styles (layered, client-server, microservices)
  - Tradeoffs and constraints
  - Architecture vs design
- Head First Software Development:
  - Chapter 5: Good-Enough Design: Getting it done with great design
  - Appendix A: Leftovers: The top 5 topics (we didn’t cover)
    - UML class diagrams
    - Sequence diagrams
    - Refactoring
- Wikipedia:
  - https://en.wikipedia.org/wiki/Software_architecture
  - https://en.wikipedia.org/wiki/Architectural_pattern
  - https://en.wikipedia.org/wiki/Monolithic_application
  - https://en.wikipedia.org/wiki/Microservices

## Lecture 16: Design Principles (SOLID and Modularity)
- SWEBOK: Software Design
- Topics:
  - Single Responsibility Principle
  - Coupling and cohesion
  - Interface boundaries
- Wikipedia:
  - https://en.wikipedia.org/wiki/SOLID
  - https://en.wikipedia.org/wiki/Coupling_(computer_programming)
  - https://en.wikipedia.org/wiki/Cohesion_(computer_science)

## Lecture 17: Software Quality (Beyond Testing)
- SWEBOK: Software Quality
- Topics:
  - Quality attributes (maintainability, reliability, usability)
  - QA vs testing
  - Static vs dynamic quality techniques
- Wikipedia:
  - https://en.wikipedia.org/wiki/Software_quality
  - https://en.wikipedia.org/wiki/ISO/IEC_25010
  - https://en.wikipedia.org/wiki/Static_program_analysis

## Lecture 18: Process and Process Maturity (Beyond Scrum)
- SWEBOK: Software Engineering Process
- Topics:
  - Limits of Scrum
  - Process visibility vs control
  - Process maturity ideas (lightweight discussion)
- Wikipedia:
  - https://en.wikipedia.org/wiki/Software_development_process
  - https://en.wikipedia.org/wiki/Capability_Maturity_Model
  - https://en.wikipedia.org/wiki/Agile_software_development

## Lecture 19: Software Engineering Economics
- SWEBOK: Software Engineering Economics
- Topics:
  - Cost vs value
  - Estimation uncertainty
  - Tradeoffs in engineering decisions
- Wikipedia:
  - https://en.wikipedia.org/wiki/Cost%E2%80%93benefit_analysis
  - https://en.wikipedia.org/wiki/Software_development_effort_estimation
  - https://en.wikipedia.org/wiki/Opportunity_cost

## Lecture 20: Professional Practice and Ethics
- SWEBOK: Software Engineering Professional Practice
- Topics:
  - Responsibility in deployed systems
  - Communication and expectations
  - Ethical failures in software systems
- Wikipedia:
  - https://en.wikipedia.org/wiki/Computer_ethics
  - https://en.wikipedia.org/wiki/Software_engineering_professional_practice
  - https://en.wikipedia.org/wiki/Ethics_of_technology
